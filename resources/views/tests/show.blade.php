		    ...
		    <div v-for="question in questions" @click="showAnswers(question.id)" class="panel panel-default" id="panel1">
		        <div class="panel-heading">
		            <h4 class="panel-title">
		        		...
		        		<button @click="createNewAnswer(question)" class=" btn btn-default btn-xs" data-title="Create" data-toggle="modal" data-target="#create-answer">New answer</button>
		      		</h4>
		        </div>
...
{{-- CREATE ANSWER MODAL --}}
<div class="modal fade" id="create-answer" tabindex="-1" role="dialog" aria-labelledby="create" aria-hidden="true">
    <div class="modal-dialog">
    	<div class="modal-content">
          	<div class="modal-header">
        		<button type="button" class="close" data-dismiss="modal" aria-hidden="true"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></button>
        		<h4 class="modal-title custom_align" id="Heading">Create a new answer</h4>
      		</div>
        	<div class="modal-body">
				<div class="form-group">
		          	<label for="answer">answer:</label>
		        	<input id="answer" v-model="newAnswer.answer" class="form-control " type="text" placeholder="Answer">
		        	<h4>Answer:</h4>
		        </div>
	    		<div class="form-group">
				    <label for="points">Points</label>
				    <select class="form-control" id="correct" v-model="newAnswer.correct">
				      	@for ($i = 0; $i <= 1; $i++)
					      	<option>{{$i}}</option>
					    @endfor
				    </select>
			  	...
			  	<div class="modal-footer ">
        			<button type="button" class="btn btn-warning btn-lg" style="width: 100%;" @click="createAnswer($event)"><span class="glyphicon glyphicon-ok-sign" ></span>Create</button>
      			</div>
      		</div>
        </div><!-- /.modal-content -->
  	</div><!-- /.modal-dialog -->
</div>
{{-- END CREATE ANSWER MODAL --}}
...
<script>
	...
	const app = new Vue({
	    ...
	    methods: {
	    	createNewAnswer: function(question) {
	    		this.newAnswer.question_id = JSON.parse(JSON.stringify(question.id));
	    	},
	    	createAnswer: function(event) {
	    		var self = this;
				this.$http.post(ajax_url + '/api/answers',this.newAnswer).then((response) => {
					$("#create-answer").modal('hide');
					new PNotify({title:"Success", text:"Successfully created new answer"});
					questionId =self.newAnswer.question_id
